	...
	?>
		...
		<div id="page-wrapper" class = "fright">
			<div class="container-fluid" id = "data">
				<div class="toprightstat">
					...
					<div class="noti">
						<text><b>EXPIRED:</b> </text><?php 
											$query = "select count(AgreementID) from client_agreement where status = 2 or status = 6";
											$exec = mysql_query($query);
											if($exec == false)
											{
												echo "Error:";
					...
					<div class="noti">
						<text><b>TERMINATED:</b> </text><?php 
											$query = "select count(AgreementID) from client_agreement where status = 3 or status = 7";
											$exec = mysql_query($query);
											if($exec == false)
											{
												echo "Error:";
				...
				<br><br>
				<text class = "tranhead"><center>CLIENT RENEWAL REQUESTS</center></text><br>
				<br><br><br><br>
				<table id="example" class="table table-hovertransac table-bordered" cellspacing="0" width="100%">
					...
					<tbody>
						<?php
							...
							$sql = "select client.*, client_agreement.ClientID, client_agreement.Status from client left join client_agreement on client.ClientID=client_agreement.ClientID where client.Status = 5 and client_agreement.Status > client.Status order by client.ApplicationDate desc";
							...
							while($row = mysql_fetch_array($exec))
							{
								...
								if ($stat == "In Progress")
								{
									...
									\"$dateSent\", \"$jobPass\", \"$coursePass\", \"$educ\", \"$numopen\", \"$urgent\", \"$limit\", \"$dateorder\", \"$id\", \"$agreementID\")'>View</button></td>";
									echo "<td>
								...
								else if ($stat == "Agreement Request")
								{
									echo "<tr>";
									echo"<td>$stat</td>";
									echo"<td>$dateSent</td>";
								...
								else if ($stat == "Termination Request")
								{
									echo "<tr>";
									echo"<td>$stat</td>";
									echo"<td>$dateSent</td>";
		...
		<script type="text/javascript">
			...
			function details(business, name, add, email, c, t, w, date, job, course, educ, op, u, l, o, id, agreement)
			{
