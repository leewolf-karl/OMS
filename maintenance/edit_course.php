...
<?php
	...
	if(isset($_POST['id'])){
		...
		$courseId = mysql_real_escape_string($_POST['id']);
    ...
    if($row >= 1){
       ...
       while($res = mysql_fetch_assoc($check)){
        ...
        $query1 = mysql_query("Update `course` Set Name = '$courseName' WHERE CourseID = $courseId;");
        ?>
          <div>
              <script>
                  $(function(){
		...
		$query2 = mysql_query("Select * from course where Status != 3 and Name != ' '");
                                    if ($query2 == FALSE){
                                        ...
                                        ?>
                                            <div>
                                                <script>
                                                  $(function(){
                                                      $(document).ready(function(){
                                                        $('input[name=edit_checkbox_<?php echo $courseID;?>]').bootstrapSwitch('state', true, true);
                                                      });
                                                  });
                                                </script>
                                            </div>
...
<tr>
                                          ...
                                          <td>
                                             <center>
                                              ...
                                              ?>
                                              <?php 
                                                  echo "<input id='switch-state' name='edit_checkbox_$courseID' type='checkbox' value='$courseID' >";
                                              ?>
                                               <?php 
